...
class MMPerceiverEncoder(nn.Module):
    def __init__(
        self,
        adapter_dict: Dict[str, InputAdapter],
        num_latents: int,
        num_latent_channels: int,
        num_cross_attention_heads: int = 4,
        num_cross_attention_qk_channels: Optional[int] = None,
        ...
        # learnable initial latent vectors
        self.latent_dict = {}
        for mod_typ in adapter_dict.keys():
            self.latent_dict[mod_typ] = nn.Parameter(torch.empty(num_latents, num_latent_channels))
        self._init_parameters(init_scale)

    def _init_parameters(self, init_scale: float):
        with torch.no_grad():
            for mod_typ in self.latent_dict.keys():
                self.latent_dict[mod_typ].normal_(0.0, init_scale)
            _init_module_parameters(self, init_scale)

    def forward(self, data_dict, pad_mask=None):
        latent_dict = {}
        for mod_typ, x in data_dict:
            b, *_ = x
            x = self.adapter_dict[mod_typ](x)
            x_latent = repeat(self.latent_dict[mod_typ], "... -> b ...", b=b)
            
            # share-weight for main-stem : CA + SA block
            x_latent = self.cross_attn_1(x_latent, x, pad_mask)
            latent_dict[mod_typ] = self.self_attn_1(x_latent)
